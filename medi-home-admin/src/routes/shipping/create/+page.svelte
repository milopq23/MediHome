<script>
	...
	import { CreateShipping } from '$lib/api/shipping.js';
	...
	async function createShipping() {
		try {
			const data = { ...shipping };
			const res = await CreateShipping(shipping);
			goto('/shipping');
		} catch (err) {
...
<div class="flex items-center justify-center py-10">
	<div class="w-full max-w-3xl rounded-2xl bg-white p-8 shadow-xl dark:bg-gray-800">
		<HeaderDetail title="Tạo shipping" route="shipping" />

		<form class="space-y-6" on:submit|preventDefault={createShipping}>
			<div>
