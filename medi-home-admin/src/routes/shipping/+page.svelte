<script>
	import { ListShipping, DeleteShipping } from '$lib/api/shipping.js';
	...
	async function listShippings() {
		const data = await ListShipping();
		shippings = data;
	}
	...
	function viewDetailShipping(id) {
		// toasts.add({ message: 'Lưu thành công', type: 'success' });
		goto(`/shipping/${id}`);
	}

	function create() {
		goto(`/shipping/create`);
	}
			...
			<table class="min-w-full divide-y divide-gray-200 text-sm">
				<thead class="bg-gray-50 text-left text-gray-500">
					<tr>
						...
						<th class="table-cell p-2">Loại vận chuyển</th>
						<th class="table-cell p-2">Giá</th>
						<th class=""></th>
					</tr>
				</thead>
				<tbody class="text-gray-800">
					{#each shippings as shipping}
						<tr class="border-b hover:bg-gray-50">
							<td class="table-cell p-2">{shipping.shipping_id}</td>
							<td class="table-cell p-2">
								{shipping.name}
							</td>
							<td class="table-cell p-2">{formatVND(shipping.price)}</td>
							<td class="p-2">
								<div class="flex items-center gap-2 text-gray-600">
									<button
										class="hidden cursor-pointer md:table-cell"
										title="Sửa"
										on:click={() => viewDetailShipping(shipping.shipping_id)}
										...
										>
											...
											<div class="mt-6 flex justify-end gap-3">
												...
												<button
													...
													on:click={() => confirmDelete(shipping.shipping_id)}
												>
													Xác nhận
												</button>
											</div>
									...
									<div>
										<div class="relative inline-block w-2 md:hidden">
											<button on:click={() => openMoreOption(shipping.shipping_id)}>
												<MoreVertical class="h-4 w-4" />
											</button>
										</div>

										{#if selectOption === shipping.shipping_id}
											<div
